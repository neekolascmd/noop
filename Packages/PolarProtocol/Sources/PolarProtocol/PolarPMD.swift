...
public enum PolarPMDSettingType: UInt8, CaseIterable, Codable, Sendable {
    ...
    /// Width of one wire value in Polar's current public SDK.
    ///
    /// Security is a 128-bit value. It is retained losslessly in `options`, but omitted
    /// from numeric selection because the convenience encoder is intentionally UInt32-bound.
    public var numericFieldWidth: Int? {
        switch self {
...
///
/// Values stay as raw little-endian byte strings so IEEE-754 factor bits are never lossy.
/// Numeric convenience accessors are provided for
...
public struct PolarPMDSettings: Equatable, Sendable {
    ...
    public init(parameters: [UInt8]) throws {
        ...
        while offset < parameters.count {
            ...
            guard let width = type.numericFieldWidth else {
                throw PolarPMDError.unsupported("setting \(type) has no published wire width")
            }
            ...
            let byteCount = count.multipliedReportingOverflow(by: width)
    ...
    public func numericOptions(_ type: PolarPMDSettingType) -> [UInt32] {
        guard let width = type.numericFieldWidth, width <= 4 else { return [] }
...
// MARK: - One-command transport gate

/// Makes the platform GATT transports robust to either legal callback order: some stacks report
/// the characteristic-write acknowledgement before the PMD indication, while others deliver the
/// indication first. A command completes only after both events arrive and match its opcode/type.
public struct PolarPMDCommandTransaction: Sendable {
    private struct Pending: Sendable {
        let command: [UInt8]
        var writeAcknowledged = false
        var response: PolarPMDControlResponse?
    }
    ...
    public var isIdle: Bool { pending == nil }
    public var activeCommand: [UInt8]? { pending?.command }

    public mutating func reset() {
        pending = nil
    }

    public mutating func begin(_ command: [UInt8]) throws {
        guard pending == nil else {
            throw PolarPMDError.malformed("control command already in flight")
        }
        guard command.count >= 2 else {
            throw PolarPMDError.truncated("control command")
        }
        pending = Pending(command: command)
    }

    /// Returns the response only when it arrived before this acknowledgement.
    public mutating func acknowledgeWrite(succeeded: Bool) throws -> PolarPMDControlResponse? {
        guard var current = pending else {
            throw PolarPMDError.malformed("unexpected control write callback")
        }
        guard succeeded else {
            pending = nil
            throw PolarPMDError.malformed("control write failed")
        }
        current.writeAcknowledged = true
    ...
    /// Returns the response only when the write was already acknowledged.
    public mutating func receive(_ response: PolarPMDControlResponse) throws -> PolarPMDControlResponse? {
        guard var current = pending else {
            throw PolarPMDError.malformed("unexpected control response")
        }
        guard Int(current.command[0]) == Int(response.opcode),
              Int(current.command[1] & 0x3F) == Int(response.measurementByte & 0x3F) else {
            pending = nil
            throw PolarPMDError.malformed("control response does not match active command")
        }
        guard current.response == nil else {
            pending = nil
            throw PolarPMDError.malformed("duplicate control response")
        }
