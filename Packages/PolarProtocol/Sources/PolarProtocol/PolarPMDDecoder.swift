...
public struct PolarPMDDecoder: Sendable {
    ...
    public static func deltaSamples(_ bytes: [UInt8],
        ...
        while offset < bytes.count {
            ...
            offset += 2
            // The official parser skips a zero-width block as malformed. Accepting it as
            // repeated samples would turn corrupt bytes into plausible motion/PPG values.
            guard (1...32).contains(deltaWidth) else {
                throw PolarPMDError.unsupported("delta width \(deltaWidth)")
            }
            guard count > 0 else {
                throw PolarPMDError.malformed("delta block has zero samples")
            }
