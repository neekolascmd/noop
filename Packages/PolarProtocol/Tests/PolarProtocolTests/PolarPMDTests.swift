...
final class PolarPMDTests: XCTestCase {
    ...
    func testSecuritySettingRetainsPublished128BitValueWithoutSelectingIt() throws {
        let security = Array(UInt8(0)..<UInt8(16))
        let settings = try PolarPMDSettings(parameters: [0x06, 0x01] + security)
        XCTAssertEqual(settings.options[.security], [security])
        XCTAssertEqual(settings.numericOptions(.security), [])
        XCTAssertNil(settings.maximumSelection[.security])
    }
    ...
    func testCommandTransactionAcceptsBothCallbackOrders() throws {
        let command = PolarPMDCommand.querySettings(.accelerometer)
        let response = try PolarPMDControlResponse(
            bytes: [0xF0, 0x01, 0x02, 0x00, 0x00]
        )

        var writeFirst = PolarPMDCommandTransaction()
        try writeFirst.begin(command)
        XCTAssertNil(try writeFirst.acknowledgeWrite(succeeded: true))
        XCTAssertEqual(try writeFirst.receive(response), response)
        XCTAssertTrue(writeFirst.isIdle)

        var responseFirst = PolarPMDCommandTransaction()
        try responseFirst.begin(command)
        XCTAssertNil(try responseFirst.receive(response))
        XCTAssertEqual(try responseFirst.acknowledgeWrite(succeeded: true), response)
        XCTAssertTrue(responseFirst.isIdle)
    }

    func testCommandTransactionRejectsMismatchFailureAndOverlap() throws {
        var transaction = PolarPMDCommandTransaction()
        try transaction.begin(PolarPMDCommand.querySettings(.ppi))
        XCTAssertThrowsError(try transaction.begin(PolarPMDCommand.querySettings(.accelerometer)))
        XCTAssertThrowsError(try transaction.receive(PolarPMDControlResponse(
            bytes: [0xF0, 0x01, 0x02, 0x00, 0x00]
        )))
        XCTAssertTrue(transaction.isIdle)

        try transaction.begin(PolarPMDCommand.querySettings(.ppi))
        XCTAssertThrowsError(try transaction.acknowledgeWrite(succeeded: false))
        XCTAssertTrue(transaction.isIdle)
    }
    ...
    func testRawAccelerationWidths() throws {
        ...
        let decoded24 = try decoder.decode(signed24)
        XCTAssertEqual(
            decoded24.acceleration.map { [$0.xMilliG, $0.yMilliG, $0.zMilliG] },
            [[-2, 0, 8_388_607]]
        )
    }
    ...
    func testMalformedPacketsAreRejected() throws {
        ...
        XCTAssertThrowsError(
            try PolarPMDSettings(parameters: [PolarPMDSettingType.security.rawValue, 1])
        )
        ...
        decoder.configure(.accelerometer, sampleRateHz: 25)
        XCTAssertThrowsError(try decoder.decode(
            frame(.accelerometer, timestamp: 1_000_000_000, frameType: 3,
