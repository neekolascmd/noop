...
func describe(_ e: OuraEvent) -> String {
    ...
    case .spo2RPI(let v):
        let tenths = v.samples.compactMap(\.ring4CalibratedTenthsPercent)
        return "SPO2_R_PI[TIER-B,EXPERIMENTAL] samples=\(v.samples.count) ring4_tenths=\(tenths) rt=\(v.ringTimestamp)"
