...
final class BackfillerSessionTallyTests: XCTestCase {
    ...
    // MARK: - WHOOP 5/MG type-52 preservation

    @MainActor func testWhoop5Type52RoutesAsOffloadButWhoop4DoesNot() {
        var whoop5 = [UInt8](repeating: 0, count: 12)
        whoop5[8] = 52
        XCTAssertTrue(BLEManager.isOffloadFrame(whoop5, family: .whoop5))

        var whoop4 = [UInt8](repeating: 0, count: 8)
        whoop4[4] = 52
        XCTAssertFalse(BLEManager.isOffloadFrame(whoop4, family: .whoop4))
    }

    private final class Type52Store: BackfillStoreWriting {
        ...
        func setCursor(_ name: String, _ value: Int) async throws { record("cursor") }
        func cursor(_ name: String) async throws -> Int? { nil }
    }

    private func whoop5Type52Frame() -> [UInt8] {
        puffinCommandFrame(cmd: 0, seq: 1, payload: [0x01, 0x02, 0x03, 0x04], type: 52)
    }

    private func whoop5HistoryEndFrame(trim: UInt32) -> [UInt8] {
        func le32(_ value: UInt32) -> [UInt8] {
            [UInt8(value & 0xff), UInt8((value >> 8) & 0xff),
             UInt8((value >> 16) & 0xff), UInt8((value >> 24) & 0xff)]
        }
        // metadata data = unix + subsec + reserved + end_data(trim + next)
        let payload = le32(1_700_000_000) + [0, 0] + le32(0) + le32(trim) + le32(trim &+ 1)
        // Current hardware-backed HISTORY_END uses type 49 inside the puffin envelope.
        return puffinCommandFrame(cmd: 2, seq: 0, payload: payload, type: 49)
    }

    @MainActor func testWhoop5Type52IsArchivedBeforeCursorAndAck() async {
        var order: [String] = []
        let store = Type52Store { order.append($0) }
        let backfiller = Backfiller(
            store: store,
            deviceId: "test",
            ackTrim: { _, _ in order.append("ack") },
            rejectedSink: { frames, _, family in
        ...
        await backfiller.ingest(whoop5Type52Frame())
        await backfiller.ingest(whoop5HistoryEndFrame(trim: 42))

        XCTAssertEqual(order, ["insert", "archive", "cursor", "ack"])
        XCTAssertEqual(backfiller.lastAckedTrim, 42)
    }

    @MainActor func testWhoop5Type52ArchiveFailureHoldsCursorAndAck() async {
        var order: [String] = []
        let store = Type52Store { order.append($0) }
        let backfiller = Backfiller(
            store: store,
        ...
        backfiller.begin(family: .whoop5)
        await backfiller.ingest(whoop5Type52Frame())
        await backfiller.ingest(whoop5HistoryEndFrame(trim: 43))

        XCTAssertEqual(order, ["insert", "archive"])
