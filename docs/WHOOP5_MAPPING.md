...
| History range / chunk ACK | Mapped | Apple and Android both request `GET_DATA_RANGE`, then start the cursor-bearing offload and acknowledge each chunk. Apple fails open after two seconds if the range response is absent. |
...
| R22 configuration | Persistent transaction observed | Fifteen documented writes receive responses, but no separate live stream exists and NOOP has no captured restore sequence. Controlled before/after firmware comparisons remain. |
...
fixture-array JSON and JSONL imports. Android keeps capture open for ten seconds after sync completion so
trailing history and command responses are not lost. All capture files contain personal biometric history
...
Repeat the identical sequence before and after R22 only if a persistent, currently unrestorable device
change is acceptable. Do not infer a field from
...
## Normal-sync qualification (2026-07-14)

An in-place Android demo update preserved the existing encrypted bond and completed an ordinary sync with
raw capture enabled. The new segment retained 737 frames, including 596 historical records: 578 v18 and 18
v26. The command trace contained `GET_DATA_RANGE`, `SEND_HISTORICAL_DATA` and their responses. Capture
remained open through the post-sync cooldown and then stayed byte-stable.

No v20, v21 or type-52 record appeared. This confirms that those layouts are not requested through separate
NOOP commands: the firmware chooses which record families to return during the normal history transaction.
The persistent R22 configuration sequence was deliberately not sent because NOOP does not yet have a
captured restore sequence. Qualifying the absent layouts therefore requires another device or firmware that
naturally emits them, or a controlled persistent before/after experiment on hardware whose state may remain
changed.
