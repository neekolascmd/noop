...
- **[oura-openapi]** - Oura's official API v2
  ...
  defines an interval plus nullable items. `PublicWorkout` defines activity, start/end, calories,
  distance, intensity, source, and label; NOOP accepts equivalent files locally and stores only those
  exported facts. It does not call the OAuth API.
...
4. Receive the `0x11` summary and inner TLV records (§6). Some Ring 4 firmware banks the fresh correlated `0x42`; tested firmware 2.12.3 instead acknowledged SyncTime and returned recent `0x85` RTC beacons without any `0x42`. During the active fetch, an RTC beacon no older than 72 hours, no more than 15 minutes ahead of the request window, and at or beyond the durable cursor may establish a persistable 100 ms/tick mapping. The 72-hour bound is hardware-qualified from a retained page whose valid beacons were about 52 hours old; the prior 48-hour gate rejected that page despite every other qualification passing. A later correlated `0x42` still replaces it. The summary can precede its record notifications, so hold it until the TLV stream has been idle for 500 ms. If a page contains records but no anchor, keep its decoded events parked and issue only another `max=255` read from the actual record high-water: no `max=0` ACK, cursor save, flush, or persistence. Each in-memory window stops at 32 pages or 100,000 parked decoded events; a real retained Ring 4 page expanded to roughly 74,000 verified events, so the previous 4,096-event cap could never commit it. Anchored persistence writes those events in 2,048-event idempotent chunks before the cursor advances, bounding the transient database batch. At the read-only bound, discard provisional RAM only and continue from the record high-water for at most eight bootstrap windows; the ring data remains unacknowledged. During that explicitly bounded bootstrap, a plausible historical `0x42` may establish the clock as open_ring does. No-progress or terminal responses still stop the scan.
...
- **`0x50` activity_info**: byte0 is retained as `state_raw` (meaning unconfirmed); every following byte is one wire-order MET bin, `met = byte × 0.1` for `byte < 0x80`, else `met = 12.8 + (byte − 128) × 0.2` (two-slope: 0.1-MET resolution to 12.7, 0.2 steps above). [ringverse][oura-rs]
  - Six real Gen 3 captures first corroborated physiologic output from steady resting (0.9–1.1 MET) through a vigorous burst (7.4 MET). The retained Ring 4 / FW 2.12.3 corpus independently contains 282 records / 1,670 bins spanning 0.9–11.9 MET, with 2–13 bins per record. For 245 of 281 adjacent transitions, the next record's bin count × 60 seconds exactly matches the ring-clock gap within one 100 ms tick; every other transition is a positive whole-minute gap, never an overlap. This qualifies the record shape, low-byte formula, and **60-second bin cadence** on Ring 4. The high-byte branch was not observed and remains prior-art-only; wire order is preserved without asserting its chronological direction.
...
- **`0x73` `ehr_trace_event`** is a banked output associated with Exercise HR feature `0x03`; its body
  remains wholly unknown and presence-only Tier B. **`0x74` `ehr_acm_intensity_event`** has a bounded
  native-parser-backed structural decoder: a non-empty even-length body of at most 14 bytes becomes up
  to seven unsigned 16-bit little-endian values in wire order. NOOP retains that atomic series as
  `OURA_EXERCISE_HR_INTENSITY_SERIES` with `unit=raw_u16`, `field_semantics=unvalidated`, and
  `timestamp_semantics=record_anchor_only`. The physical unit, cadence, and relationship to workout HR
  remain unknown; it never becomes HR, strain, or a workout metric. Neither tag appeared in the retained
  Ring 4 corpus. [open_oura-feat][oura-rs]
- **`0x7E`/`0x7F` real_steps_features 1/2** have an exact 14-byte body (20-byte complete TLV including
  the six-byte record header). The native-parser-backed unpacker yields 14 ordered unsigned fields:
  `[(p3>>7)|(p0<<1), p1<<1, p2<<1, p3&0x7f, p4, p5, p6, p7,`
  `(p11>>7)|(p8<<1), p9<<1, p10<<1, p11&0x7f, p12, p13]`. Field names and the stateful relationship
  between parts 1 and 2 remain unvalidated. NOOP therefore retains each exact record as
  `OURA_REAL_STEPS_FEATURES`, including `source_tag`, `feature_fields_u16`, and
  `part_relationship=stateful_unknown`, but never converts any field into a steps row. Neither tag
  appeared in the retained Ring 4 corpus. [oura-rs]
  ...
  - `AUTOMATIC` means the ring computes while worn and banks events for NOOP's automatic history sync; it
    ...
    `0x7E`/`0x7F`, NOOP keeps step counts blank while preserving their structural diagnostics for later
    reference correlation. Decoder revision 6 can recover both activity diagnostic families from archived
    TLVs without reconnecting the ring.
...
| `0x03` | Exercise HR (AWHR) | Gen3+; cap version ≥ 2. NOOP's explicit activity action enables this only after Real Steps, then re-reads both statuses. `0x73` remains Tier B; structurally decoded `0x74` remains a non-metric diagnostic until captured and reference-correlated. |
...
- Validate all Tier-B and structurally diagnostic sleep/activity/step layouts against real captures and
  independent references before enabling any unresolved field in scoring.
