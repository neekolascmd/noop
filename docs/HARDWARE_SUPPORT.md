...
| Garmin v2 local sync | Android bonding/GATT transport for Multi-Link `6A4E2800…`, paired `2810…2814` / `2820…2824` characteristics, handle negotiation, COBS/GFDI session replies, and conservative realtime HR, steps, HRV, SpO₂, and respiration persistence. Swift and Kotlin protocol cores share synthetic vectors. Accelerometer and Body Battery remain opaque and are not presented as decoded metrics | **Implemented / experimental / hardware-unverified** — software tests and cached host compilation pass; there is no recorded owned-watch connection, handle-registration trace, or biometric stream result | Record exact watch model, firmware, Android device/OS, GATT pair, bond result, assigned handles, and one privacy-redacted sample count per stream. Apple has a protocol package only; app transport/routing is not implemented |
...
| Amazfit / Zepp deep BLE | Vendor BLE history and sensors | **Planned** | Needs owned hardware and clean-room captures; no current device inventory recorded |
...
Garmin's local-sync implementation and promotion checklist are documented in
[Garmin protocol](GARMIN_PROTOCOL.md). Software-only parser tests do not move a watch into a verified
hardware cell.
