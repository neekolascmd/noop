...
| Oura Ring 4 overnight history qualification | Same available Ring 4 tuple as above | MacBook Neo, macOS 27.0 | NOOP Oura qualification branch | **2026-07-13** | An overnight-worn ring completed the bounded Ring 4 anchor bootstrap/refetch and materially increased banked temperature, HRV, and R-R rows with plausible anchored timing. The then-running build had already advanced the durable history cursor before the newly-added `0x76` bedtime-boundary and lossless `0x6F` percentage decoders existed, so those records cannot be replayed without resetting user history state (NOOP does not do that). The next overnight must run the new build. Read-only feature status also confirmed automatic SpO2 was off; the app now offers an explicit, confirmed opt-in rather than silently changing the ring setting. Sleep stages remain unqualified. |
...
| Oura | Local BLE | **Partial / experimental** — Ring 4 on Android has a recorded factory-reset adoption, stored-key authentication, 30-minute live HR/R-R run, app-process relaunch, battery, and history/skin-temperature fetch. The same ring now also has a recorded macOS adoption/relaunch/live run and an overnight history pass that persisted substantially more temperature, HRV, and R-R data. The production path now preserves Ring 4 bedtime bounds as stage-less sleep sessions and accepts only qualified percentage SpO2 records. | Ring 4 remains available. The next overnight must qualify the new bedtime/percentage code after explicitly enabling automatic SpO2. Android and macOS still need a reference sensor, real out-of-range reconnect, and a full 24-hour run; the second available ring needs an independent tuple. No physical iPhone/iOS result is recorded, and sleep stages remain experimental. |
