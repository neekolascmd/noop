...
| Oura | Local BLE | **Partial / experimental** — Ring 4 on Android has a recorded factory-reset adoption, stored-key authentication, 30-minute live HR/R-R run, app-process relaunch, battery, and history/skin-temperature fetch. The same ring now also has a recorded macOS adoption/relaunch/live run and overnight passes. A retained-history pass qualified the ring's roughly 52-hour-old RTC beacon under a bounded 72-hour fallback, persisted a roughly 74,000-event page in idempotent chunks, and durably advanced the cursor while withholding an implausibly timed sample. Automatic SpO2 was on, but the banks contained no qualified percentage record, so the missing value stayed honest. The software now also losslessly decodes `0x8B` ratio/perfusion records and preserves explicitly profiled per-sample calibration evidence in a diagnostic event; it does not promote those estimates to the production oxygen metric or Health Connect. Ordered `0x4E`/`0x5A` phase records are likewise preserved atomically with the corrected deep/light/REM/awake codebook, but no cadence is assumed and they do not feed staging. The prior Ring 4 bank emitted none of those diagnostic records. | Ring 4 remains available. Run a fresh overnight inventory to qualify `0x8B` on that exact tuple, then compare the labelled estimates with a reference sensor. Capture any `0x4E`/`0x5A` record alongside the official app hypnogram to qualify cadence/direction. Android and macOS still need a real out-of-range reconnect and a full 24-hour run; the second ring needs an independent tuple. No physical iPhone/iOS result is recorded, and sleep stages plus SpO2 remain experimental. |
