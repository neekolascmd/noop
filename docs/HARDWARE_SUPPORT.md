...
| Polar H10 / Verity Sense / OH1 deep streams | PMD PPI + ACC live integration; ECG / PPG decode boundary | **Implemented / unverified** — clean-room Swift and Kotlin protocol twins are tested; Apple and Android builds automatically negotiate PPI + ACC while preserving standard HR/battery on PMD failure. ECG/PPG decoding is implemented but not started or stored because the durable waveform lane is not built. Issue `#421` remains hardware-gated | No named device/firmware tuple is recorded. Run the checklist in [Polar PMD qualification](POLAR_PMD.md) separately for H10, Verity Sense, and OH1 before changing this row to Partial or Verified |
