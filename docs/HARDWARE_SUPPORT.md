...
|---|---|---|---|
| Generic BLE heart-rate devices (Polar, Wahoo, Coospo, Garmin HRM, Amazfit HR broadcast) | Standard `0x180D` / `0x2A37` live HR and R-R; bounded automatic reconnect after an involuntary drop | **Implemented** since v3.8.0; no per-model firmware matrix is recorded | Devices not inventoried. Report each model separately before calling it verified |
