...
| Oura | Local BLE | **Partial / experimental** — Ring 4 on Android has a recorded factory-reset adoption, stored-key authentication, 30-minute live HR/R-R run, app-process relaunch, battery, and history/skin-temperature fetch. The same ring now also has a recorded macOS factory-reset adoption, key persistence, fresh-process stored-key authentication, live-mode enable, battery/history fetch, and a continuous 30-minute worn HR/R-R run. | Ring 4 remains available. Android and macOS still need a reference sensor, real out-of-range reconnect, overnight/24-hour runs, and a second independent ring. No physical iPhone/iOS result is recorded. |
