...
| Oura | Local BLE | **Partial / experimental** — Ring 4 on Android now has a recorded factory-reset adoption, stored-key authentication, 30-minute live HR/R-R run, app-process relaunch, battery, and history/skin-temperature fetch in addition to advertisement/GATT/identity evidence | Ring 4 remains available on a Solana Mobile Saga. Still needs comparison with a simultaneous reference sensor, a real out-of-range reconnect, the overnight/24-hour run, and a second independent ring before Supported |
