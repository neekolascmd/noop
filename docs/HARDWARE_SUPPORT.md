...
| Oura | Local BLE | **Partial / experimental** — Ring 4 on Android now has a recorded factory-reset adoption, stored-key authentication, live HR/R-R, battery, and history/skin-temperature fetch in addition to advertisement/GATT/identity evidence | Ring 4 remains available on a Solana Mobile Saga. Still needs the 30-minute worn session, app relaunch, out-of-range reconnect, overnight/24-hour run, and a second independent ring before Supported |
