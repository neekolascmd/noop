...
| Oura Ring 4 overnight history qualification | Same available Ring 4 tuple as above | MacBook Neo, macOS 27.0 | NOOP Oura qualification branch | **2026-07-14** | A second overnight-worn run used the new decoder build. After a safe reconnect established the Ring 4 UTC anchor, NOOP decoded and persisted one plausible `0x76` bedtime window lasting **7 h 18 min** as an intentionally stage-less sleep session; no history cursor was reset. Read-only status confirmed automatic SpO2 was on, but the ring emitted no qualified `0x6F` percentage, stable, smoothed, or new raw-DC SpO2 record in that overnight bank, so NOOP correctly displayed no value. A later exact-build reconnect authenticated and streamed successfully but correctly kept its batch provisional when the ring supplied no fresh primary anchor. The retained-history RTC qualification below closes that transport gap without weakening the durable cursor or timestamp checks. That bank emitted no `0x4B`/`0x4E`/`0x5A`, so revision 8 does not retroactively qualify Ring 4 stages. |
| Oura Ring 4 retained-history RTC and large-page commit | Same available Ring 4 tuple as above | MacBook Neo, macOS 27.0 | NOOP PR `#14` qualification build | **2026-07-17** | A saved-key reconnect authenticated, read battery, and fetched the retained overnight bank after the phone released its BLE connection. Three structurally valid `0x85` RTC beacons were about 52 hours old, beyond the former 48-hour bound but within the new hardware-qualified 72-hour bound. The page contained 51,020 TLV records and expanded to roughly 74,000 verified decoded events, so NOOP persisted it in 2,048-event idempotent chunks before advancing the durable cursor from **2026-07-14 19:08:26 UTC** to **2026-07-16 06:15:26 UTC**. The local database gained 62,135 R-R intervals, 2,268 skin-temperature samples, and two stage-less sleep windows; no history HR samples were present, and one implausibly timed sample was withheld. The terminal caught-up response completed without claiming an unobserved explicit ACK. Battery displayed 100%. No qualified SpO2 percentage or sleep-stage records were emitted. |
| Oura Ring 4 motion-record qualification | Same available Ring 4 tuple as above | MacBook Neo, macOS 27.0 | Opt-in NOOP history capture from 2026-07-17, analyzed by the cross-platform decoder candidate | **2026-07-30** | Privacy-safe shape/cadence analysis found 2,538 `0x47` motion records using only the native 4/5/6-byte payload shapes and 1,875 `0x72` sleep-ACM records using the exact 12-byte payload. Both streams were nominally 30 seconds apart with no duplicate ring timestamps. Swift and Kotlin now decode and retain timestamped, units-labelled diagnostic events from those records, including offline replay from the bounded raw archive. They remain outside steps, sleep staging, and scoring until their semantic fields are reference-correlated. |
| Oura Ring 4 activity-MET qualification | Same available Ring 4 tuple as above | MacBook Neo, macOS 27.0 | Retained 2026-07-17 history capture, replayed by decoder revision 5 | **2026-07-31** | The capture contains 282 `0x50` records / 1,670 MET bins spanning 0.9–11.9 MET. Payloads contain 2–13 bins after one raw state byte. Of 281 adjacent record transitions, 245 match the next record's bin count × 60 seconds within one 100 ms ring tick; every mismatch is a positive whole-minute gap and none overlaps. This qualifies the low-byte MET formula, record shape, and one-minute cadence on Ring 4; the high-byte branch was not observed and remains prior-art-only. Apple and Android preserve each complete wire-order series as `OURA_ACTIVITY_MET_SERIES` at its real record anchor and can recover it from archived TLVs. The raw state meaning and whether the anchor names the first or last bin remain unresolved, so NOOP does not manufacture per-bin timestamps, steps, calories, workouts, or scores. No `0x51`, `0x52`, `0x7E`, or `0x7F` records appeared in this capture. |
| Oura Ring 4 `0x8B` ratio/perfusion qualification | Same available Ring 4 tuple as above | MacBook Neo, macOS 27.0 | Opt-in NOOP history capture from 2026-07-17, replayed by decoder revision 4 | **2026-07-30** | Re-analysis found 12,852 complete `0x8B` records / 51,397 samples. Every body matched `1 + 3n`, record timestamps were unique, and the independently decoded eight-hour `0x76` window contained 27,285 calibrated samples at about 0.95 samples/s. The Ring 4 / Oreo Oura Simple aggregate was about 97.0%. Swift and Kotlin retain the lossless ratio/PI arrays plus one real-timestamp record mean; nightly analytics requires a 30-minute dense sleep-window series and marks the result `≈` / **Oura estimate**. Firmware percentage wins, and estimates do not leave NOOP through HealthKit, Health Connect, or WHOOP-format CSV. This qualifies emission, wire shape, and offline software behavior—not clinical accuracy; simultaneous reference-sensor comparison remains open. |
...
|---|---|---|---|
| Generic BLE heart-rate devices (Polar, Wahoo, Coospo, Garmin HRM, Amazfit HR broadcast) | Standard `0x180D` / `0x2A37` live HR and R-R; bounded automatic reconnect after an involuntary drop; one-at-a-time Android GATT setup with rejection retries and timeout recovery | **Implemented** since v3.8.0; no per-model firmware matrix is recorded | Devices not inventoried. Report each model separately before calling it verified |
| Bluetooth FTMS gym equipment | Standard `0x1826` treadmill, indoor-bike, rower, and cross-trainer live metrics; one-at-a-time Android GATT setup plus bounded reconnect | **Implemented / unverified** — decoders and host builds are tested, but no named machine tuple is recorded | Record each machine model, firmware, host, and available data fields separately |
| Amazfit / Zepp / Mi Band live HR | Preferred standard `0x180D` HR or auth-free Huami custom HR on Android; one-at-a-time setup plus bounded reconnect | **Experimental / unverified** — the app reports an honest pairing requirement when the characteristic is gated | Needs owned hardware; encrypted Huami pairing and history are not implemented |
...
| Oura | Local BLE | **Partial / experimental** — Ring 4 on Android has a recorded factory-reset adoption, stored-key authentication, 30-minute live HR/R-R run, app-process relaunch, battery, and history/skin-temperature fetch. The same ring has a recorded macOS adoption/relaunch/live run, overnight passes, and bounded retained-history replay. Re-analysis of that opt-in archive hardware-qualified 12,852 `0x8B` ratio/perfusion records. NOOP now computes a qualified local Ring 4 Oura Simple nightly estimate completely offline, keeps every raw/calibrated sample for future replay, and labels the daily value `≈` / **Oura estimate**. It never exports the estimate as a measured oxygen value; a firmware percentage remains authoritative. Apple and Android also have an explicit direct-BLE activity action that enables and verifies Real Steps + Exercise HR background modes; this software path does not import a file, reset/re-key the ring, or enable research features. The app now live-decodes and durably retains native-parser-backed `0x74` intensity, `0x7E`/`0x7F` feature, and `0x86` always-on-HR diagnostic series, including offline replay from archived TLVs. Decoder revision 8 also reconstructs externally evidenced Gen 3 SleepNet `0x4B`/`0x4E`/`0x5A` bursts into 30-second staged nights from the bounded archive after a caught-up history pull, without a manual import. Staged persistence requires the complete code-slot span to cover the raw anchored `0x49` window; observed coverage below 95% preserves honest gaps but omits efficiency. Pairing requires both ring-clock and envelope Unix-time proximity, and replay splits conservatively across anchor/receipt cohorts. Each replay snapshots an archive high-water and compare-and-sets its anchor/revision snapshot so concurrent tails or anchor enrichment remain pending. Later non-SleepNet anchor evidence reopens safely related withheld rows. A newly persisted night immediately refreshes and re-scores the local read model. The owned Ring 4 archive emitted no phase records, so this is implemented software rather than Ring 4 evidence. Remaining activity field meanings are unvalidated, `0x73` remains presence-only, and none feed HR, steps, or workouts. | Ring 4 remains available. On the activity-enabled build, confirm both mode readbacks show On, walk a known route/count, allow the automatic history pass, and record whether `0x73`/`0x74`, `0x7E`/`0x7F`, and `0x86` appear. Compare activity diagnostics with a known step count and simultaneous exercise-HR reference; compare `0x86` BPM/quality pairs with simultaneous live HR before promoting them. Compare the labelled SpO2 estimate with a simultaneous reference sensor. For staging, capture `0x4B`/`0x4E`/`0x5A` and nearby anchored `0x49` records on Ring 4, then compare the automatically replayed timeline and `0xFF` gaps with a same-night official-app reference. Android and macOS still need a real out-of-range reconnect and a full 24-hour run; the already prepared second ring needs an independently recorded tuple. No physical iPhone/iOS result is recorded. Ring 4 sleep stages and measured/clinical SpO2 remain experimental. |
| Oura / Fitbit / Garmin | Offline wellness-export import | **Implemented** with shared Swift/Kotlin fixtures. Oura API-shaped JSON and sleep-period CSV import the official 5-minute phase string as a chronological hypnogram when present. Oura `heartrate.csv`, an official API-shaped discrete-HR response, and a sleep period's interval/items HR series import as one bounded, deduplicated measured stream under `oura-import`. Official Oura workout JSON/CSV summaries import start/end, activity, calories, distance, intensity, source, and label into local workout history without inventing HR, strain, zones, or routes. This verifies parsers, not current vendor hardware or export apps | Capture a fresh, privacy-redacted Oura export containing `sleep_phase_5_min`, timestamped HR, and workouts; record the app/export version, and use it as the reference timeline for direct-BLE qualification |
| Polar H10 / Verity Sense / OH1 deep streams | PMD PPI + ACC + ECG / PPG live integration | **Implemented / unverified** — clean-room Swift and Kotlin protocol twins are tested; Apple and Android builds negotiate every advertised supported stream while preserving standard HR/battery on PMD failure. ECG/PPG are retained locally in five-second lossless signed-Int32 chunks, bounded per device to the newest 24 hours and 64 MiB. Issue `#421` remains hardware-gated | No named device/firmware tuple is recorded. Run the checklist in [Polar PMD qualification](POLAR_PMD.md) separately for H10, Verity Sense, and OH1 before changing this row to Partial or Verified |
...
Tier-B and structurally diagnostic activity, step, and sleep-summary layouts may remain experimental after
a live/history transport cell graduates; they are separately fixture/reference-gated and cannot feed
scoring until validated.

The direct activity qualification is not an import test: enable **Automatic activity tracking** on the
active Oura card, verify the UI reads both ring modes back as **On**, wear the ring for a known walk, and
leave NOOP connected for its automatic history pass. Export the privacy-redacted strap log afterward; the
inventory must show whether `0x73`/`0x74`, `0x7E`/`0x7F`, and `0x86` were emitted before any decoder is promoted.
