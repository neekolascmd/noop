...
| **Polar deep streams** (ECG / PPG / ACC / PPI) | 🧪 Implemented, hardware qualification pending | Clean-room PMD protocol/decoder plus automatic Apple and Android live setup (below) |
...
| **Oura** | 🧪 Experimental; Ring 4 partially qualified on Android and macOS | Local BLE protocol package and platform sources, including direct automatic history sync and an explicit Real Steps + Exercise HR mode opt-in; exact step/exercise layouts remain capture-gated |
...
## Polar Measurement Data (PMD) — implemented, hardware qualification pending

Source: the official [`polarofficial/polar-ble-sdk`](https://github.com/polarofficial/polar-ble-sdk).
NOOP independently implements the documented wire behavior and does not link or copy the Polar SDK.
The lane can read supported PMD streams from a Polar H10 / Verity Sense / OH1 the user owns,
account-free, on top of the standard heart-rate service.
...
- **Measurement-type codes (u8):** ECG `0`, PPG `1`, ACC `2`, PPI `3`, GYRO `5`,
  MAGNETOMETER `6`, skin temperature `7`, SDK mode `9`, location `10`, pressure `11`,
  temperature `12`, offline recording `13`, offline HR `14`, and derived data `15`
  (measurement mask `0x3F`).
...
- **Data frame:** `data[0]` = measurement type; `data[1..8]` = 64-bit little-endian device timestamp;
  `data[9]` = frame type (`& 0x7F` = type, `& 0x80` = delta-compressed); payload starts at `data[10]`.
  NOOP uses Polar's 2000-01-01 epoch only when it is plausible against receive time, otherwise it
  keeps relative timing and anchors the live session to the host clock.
  - **ECG** type-0: 24-bit signed µV samples.
  - **ACC** raw type-0/1/2: 8/16/24-bit signed X/Y/Z (milli-g); unsupported frame types fail closed.
...
reconnect; Android routes PMD setup and writes through the existing one-operation GATT queue.

See [Polar PMD qualification](POLAR_PMD.md) for the exact implemented boundary and the physical-device
graduation checklist. Issue `#421` remains open until the new path is reproduced on a named H10 tuple.
