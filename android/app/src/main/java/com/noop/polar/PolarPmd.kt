...
enum class PolarPmdSettingType(
    val wire: Int,
    /** Wire width; values wider than four bytes are retained raw and omitted from numeric selection. */
    val numericFieldWidth: Int?,
    val responseOnly: Boolean = false,
) {
    SAMPLE_RATE(0, 2),
    RESOLUTION(1, 2),
    RANGE(2, 2),
...
class PolarPmdSettings private constructor(
    ...
    fun numericOptions(type: PolarPmdSettingType): List<Long> {
        val width = type.numericFieldWidth ?: return emptyList()
        if (width > 4) return emptyList()
    ...
    companion object {
        ...
        fun parse(parameters: ByteArray): PolarPmdSettings {
            ...
            while (offset < parameters.size) {
                ...
                val width = type.numericFieldWidth
                    ?: throw PolarPmdException("setting ${type.name} has no published wire width")
                ...
                parsed[type] = List(count) {
                    val value = parameters.copyOfRange(offset, offset + width)
                    offset += width
                    value
                }
            }
        ...
        fun encodeSelection(selected: Map<PolarPmdSettingType, Long>): ByteArray {
            ...
            for (type in selected.keys.sortedBy { it.wire }) {
                ...
                val width = type.numericFieldWidth
                    ?: throw PolarPmdException("unsupported numeric ${type.name}")
                if (width > 4) throw PolarPmdException("unsupported numeric ${type.name}")
                val value = selected.getValue(type)
                val max = if (width == 4) 0xFFFF_FFFFL else (1L shl (width * 8)) - 1
                ...
                output += 1
                repeat(width) { index -> output += (value ushr (index * 8)).toByte() }
            }
...
/**
 * One-command transport gate. Android and Apple may deliver the write acknowledgement and PMD
 * indication in either order; a transaction completes only after both match the active command.
 */
class PolarPmdCommandTransaction {
    private data class Pending(
        val command: ByteArray,
        var writeAcknowledged: Boolean = false,
        var response: PolarPmdControlResponse? = null,
    )

    private var pending: Pending? = null

    val isIdle: Boolean get() = pending == null
    val activeCommand: ByteArray? get() = pending?.command?.copyOf()

    fun reset() {
        pending = null
    }

    fun begin(command: ByteArray) {
        if (pending != null) throw PolarPmdException("control command already in flight")
        if (command.size < 2) throw PolarPmdException("truncated control command")
        pending = Pending(command.copyOf())
    }

    fun acknowledgeWrite(succeeded: Boolean): PolarPmdControlResponse? {
        val current = pending ?: throw PolarPmdException("unexpected control write callback")
        if (!succeeded) {
            pending = null
            throw PolarPmdException("control write failed")
        }
        current.writeAcknowledged = true
        return current.response?.also { pending = null }
    }

    fun receive(response: PolarPmdControlResponse): PolarPmdControlResponse? {
        val current = pending ?: throw PolarPmdException("unexpected control response")
        if (current.command[0].u8() != response.opcode ||
            (current.command[1].u8() and 0x3F) != (response.measurementByte and 0x3F)
        ) {
            pending = null
            throw PolarPmdException("control response does not match active command")
        }
        if (current.response != null) {
            pending = null
            throw PolarPmdException("duplicate control response")
        }
    ...
    companion object {
        ...
        fun deltaSamples(
            ...
            while (offset < bytes.size) {
                ...
                offset += 2
                // Zero-width blocks are malformed in Polar's current parser. Treating them as
                // repeated values would turn corrupt payloads into plausible health samples.
                if (width !in 1..32) throw PolarPmdException("unsupported delta width")
                if (count == 0) throw PolarPmdException("delta block has zero samples")
