 ...
 *   unix_s,iso_utc,device_id,stream,hr_bpm,rr_ms,grav_x,grav_y,grav_z,step_counter,ppg_bpm,ppg_conf,
...
object RawSensorExport {
    ...
    /** 19 columns, in the contract order shared with the Swift exporter. */
    ...
    internal suspend fun writeCsv(
        ...
        counts["hr"] = hr.size
        for (s in hr) rows += LineRow(s.ts, line(deviceId, "hr", s.ts, 0 to n(s.bpm)))

        val rr = repo.rrIntervals(deviceId, from, to, limit)
        counts["rr"] = rr.size
        for (s in rr) rows += LineRow(s.ts, line(deviceId, "rr", s.ts, 1 to n(s.rrMs)))

        val grav = repo.gravitySamples(deviceId, from, to, limit)
        counts["gravity"] = grav.size
        for (s in grav) rows += LineRow(s.ts, line(deviceId, "gravity", s.ts, 2 to n(s.x), 3 to n(s.y), 4 to n(s.z)))

        val steps = repo.stepSamples(deviceId, from, to, limit)
        counts["steps"] = steps.size
        for (s in steps) rows += LineRow(s.ts, line(deviceId, "steps", s.ts, 5 to n(s.counter)))

        val ppg = repo.ppgHrSamples(deviceId, from, to, limit)
        counts["ppghr"] = ppg.size
        for (s in ppg) rows += LineRow(s.ts, line(deviceId, "ppghr", s.ts, 6 to n(s.bpm), 7 to n(s.conf)))

        val spo2 = repo.spo2Samples(deviceId, from, to, limit)
        counts["spo2"] = spo2.size
        for (s in spo2) rows += LineRow(s.ts, line(deviceId, "spo2", s.ts, 8 to n(s.red), 9 to n(s.ir)))

        val skin = repo.skinTempSamples(deviceId, from, to, limit)
        counts["skintemp"] = skin.size
        for (s in skin) rows += LineRow(s.ts, line(deviceId, "skintemp", s.ts, 10 to n(s.raw)))

        val resp = repo.respSamples(deviceId, from, to, limit)
        counts["resp"] = resp.size
        for (s in resp) rows += LineRow(s.ts, line(deviceId, "resp", s.ts, 11 to n(s.raw)))
        ...
        for (s in sleepState) rows += LineRow(s.ts, line(deviceId, "band_sleep_state", s.ts, 12 to n(s.state)))

        val events = repo.events(deviceId, from, to, limit)
        counts["event"] = events.size
        for (s in events) rows += LineRow(
            s.ts,
            line(deviceId, "event", s.ts, 13 to WhoopCsvExporter.csvField(s.kind), 14 to WhoopCsvExporter.csvField(s.payloadJSON)),
        )
    ...
    /** Build one CSV line: provenance prefix + the 15 value cells (only [set] indices filled). */
    internal fun line(deviceId: String, stream: String, ts: Long, vararg set: Pair<Int, String>): String {
        val sb = StringBuilder(96)
        sb.append(ts).append(',').append(iso(ts)).append(',')
            .append(WhoopCsvExporter.csvField(deviceId)).append(',').append(stream)
        for (i in 0 until 15) {
            sb.append(',')
    ...
    suspend fun export(context: Context, repo: WhoopRepository, deviceId: String = "my-whoop") {
        runCatching {
            ...
            val counts = file.bufferedWriter().use { w ->
                ...
                w.append("# Device: ${WhoopCsvExporter.csvField(deviceId)}\n")
            ...
            val summary = if (total == 0) {
                "No samples in the last 24h - wear the active device and let it sync, then export again."
            } else {
