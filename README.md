...
|---|---|---|
| **Oura Ring 4** | Stored-key BLE auth, live HR + R-R, history/skin-temp fetch, overnight sleep windows, lossless `0x8B` ratio/perfusion replay, and a qualified fully-offline nightly Oura Simple estimate marked `≈` / **Oura estimate** — on macOS and Android ([hardware evidence](docs/HARDWARE_SUPPORT.md)) | Compare estimates with a simultaneous reference sensor; out-of-range reconnect and a full 24 h run. Sleep stages and measured/clinical SpO₂ remain experimental. Ring 3 and physical iOS are not yet recorded. |
...
| **Oura / Fitbit / Garmin exports** | Offline wellness-export import with shared Swift/Kotlin fixtures; Oura's official `sleep_phase_5_min` becomes a chronological hypnogram, timestamped exported HR becomes a measured local stream, and official workout summaries appear in workout history | Capture fresh exports per vendor app version. |
